...
def ensure_required_columns(field_names: list[str], required_columns: list[str], label: str) -> None:
    ...
    if missing_columns:
        raise click.ClickException(f"Missing required columns in {label}: {', '.join(missing_columns)}")
    ...
    with spec.path.open("r", newline="") as input_handle:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            if row_key in rows_by_key:
                ...
                raise click.ClickException(f"Duplicate match key in {spec.label} at row {row_number}: {formatted_key}")
...
def create_hgvs_normalizer() -> Callable[[str], str]:
    ...
    try:
        ...
        raise click.ClickException(f"The hgvs package is required for --normalize-indels: {exc}")
    ...
    try:
        ...
        raise click.ClickException(f"Failed to initialise the HGVS normaliser (is UTA reachable?): {exc}")
...
@click.option(
    "--a-separator", default="|", show_default=True, type=str, help="Joined reverse-translation separator in A."
)
@click.option(
    "--b-separator", default="|", show_default=True, type=str, help="Joined reverse-translation separator in B."
)
...
@click.option(
    ...
    multiple=True,
    type=str,
    help=("Additional columns that must match between A and B. Accepts comma-separated values and may be repeated."),
)
@click.option(
    "--ignore-reference-differences",
    is_flag=True,
...
@click.option(
    "--a-prefix", default="a_", show_default=True, type=str, help="Prefix for A core columns in differences output."
)
@click.option(
    "--b-prefix", default="b_", show_default=True, type=str, help="Prefix for B core columns in differences output."
)
@click.option(
...
def main(
    ...
    include_additional_columns = (
        pass_through_all_columns or bool(a_selected_additional_columns) or bool(b_selected_additional_columns)
    )
