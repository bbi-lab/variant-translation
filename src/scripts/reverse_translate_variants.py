...
def apply_cds_edit(coding_sequence: str, edit_start_index: int, deleted_length: int, inserted_sequence: str) -> str:
    return coding_sequence[:edit_start_index] + inserted_sequence + coding_sequence[edit_start_index + deleted_length :]
...
def join_variant_rows(
    ...
    if len(hgvs_cs) != len(hgvs_gs):
        raise click.ClickException("Internal error: mismatch between hgvs_c and hgvs_g candidate counts for a row.")
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help=("Output one row per input row by joining multiple variant_type/hgvs_c/hgvs_g values with --join-delimiter."),
)
@click.option(
    "--join-delimiter",
    default="|",
...
@click.option(
    "--output", type=click.Path(dir_okay=False, path_type=Path), default=None, help="Optional TSV output path."
)
@click.option(
    "--errors",
    "errors_output",
...
def main(
    ...
    try:
        ...
        for column_name in (transcript_column, uniprot_column, hgvs_p_column):
            if column_name and column_name in reader.fieldnames and column_name not in core_input_columns:
        ...
        if total_rows_skipped_missing_hgvs_p > 0:
            click.echo(
                f"Warning: Skipped {total_rows_skipped_missing_hgvs_p} row(s) with missing HGVS p. values.",
                err=True,
            )
